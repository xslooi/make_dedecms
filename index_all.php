...
foreach($file_list as $key=>$item){
    ...
    //格式化HTML代码 暂时去掉 脚本 页面样式 和 样式链接
    $html_body = preg_replace('/<script[\s|>][\s\S]*?<\/script>/i', '', $html_body); //js 替换为空
    $html_body = preg_replace('/<style[\s|>][\s\S]*?<\/style>/i', '', $html_body); //css  替换为空
    $html_body = preg_replace('/<link [\s|>][\s\S]*?>/i', '', $html_body); //css 链接 替换为空
...
//                    exit;
                ...
                $result_body = str_replace(BR . $v, '', $result_body);
            }
        }
    }
